    ...
    if items_file and template_file:
        if st.button("🚀 Generate Distribution"):
            try:
                ...
                # Map Column A (Cleaned) to Row Number
                ...
                # Store for Sidebar Search
                st.session_state['wb_final'] = wb_out
                st.session_state['summary_data'] = browser_summary_data
                ...
                st.download_button("📥 Download Smart Batcher Excel", output.getvalue(), "Smart_Batcher_Results.xlsx")
...
if 'wb_final' in st.session_state:
    st.sidebar.divider()
    st.sidebar.header("🔍 Find Participant")
    search_q = st.sidebar.text_input("Search Name:").strip().lower()
    
    if search_q:
        results = []
        wb = st.session_state['wb_final']
            ...
            for r_idx, row in enumerate(ws.iter_rows(values_only=True), 1):
                for c_idx, val in enumerate(row):
                    if val and search_q in str(val).lower():
                        # Try to find the team name (it's the header in the data column + 1)
                        # We look at the row where headers are bolded (simplified logic)
                        results.append({"Name": val, "Class": sn})
...
with tab2:
    ...
    st.write("Smart Batcher automates randomization and distribution.")
    st.info("**File A (Participants):** Columns per class. **File B (Template):** Column A has Class Name, others have Teams.")
    try:
        with open("User_Manual.pdf", "rb") as f:
            st.download_button("📥 Download PDF Manual", f, "Manual.pdf")
    except:
        st.caption("PDF Manual not found.")
